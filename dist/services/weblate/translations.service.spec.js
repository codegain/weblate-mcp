...
const sdk = require("../../client/sdk.gen");
describe('WeblateTranslationsService', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [
                translations_service_1.WeblateTranslationsService,
                {
                    provide: weblate_client_service_1.WeblateClientService,
                    useValue: { getClient: jest.fn().mockReturnValue(mockClient) },
                },
            ],
        }).compile();
        service = module.get(translations_service_1.WeblateTranslationsService);
    });
    afterEach(() => {
    ...
    describe('createTranslationUnit', () => {
        it('should create a monolingual unit with key and value', async () => {
            const mockUnit = {
                ...
                source: ['Hello'],
                target: [],
                web_url: 'https://weblate.example.com/translate/my-project/my-component/en/?checksum=abc123',
            };
            jest
                .spyOn(sdk, 'translationsUnitsCreate')
                .mockResolvedValue({ data: mockUnit, error: undefined });
            const result = await service.createTranslationUnit('my-project', 'my-component', 'en', { key: 'greeting', value: ['Hello'] });
        ...
        it('should create a bilingual unit with context, source, and target', async () => {
            const mockUnit = {
                ...
                source: ['Hello'],
                target: ['Hola'],
                web_url: 'https://weblate.example.com/translate/my-project/my-component/es/?checksum=def456',
            };
            jest
                .spyOn(sdk, 'translationsUnitsCreate')
                .mockResolvedValue({ data: mockUnit, error: undefined });
            const result = await service.createTranslationUnit('my-project', 'my-component', 'es', { context: 'greeting_message', source: ['Hello'], target: ['Hola'] });
            expect(result).toEqual(mockUnit);
            expect(sdk.translationsUnitsCreate).toHaveBeenCalledWith({
                client: mockClient,
                path: {
        ...
        it('should pass state parameter when provided', async () => {
            ...
            expect(sdk.translationsUnitsCreate).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should throw on API error response', async () => {
            ...
            await expect(service.createTranslationUnit('proj', 'comp', 'en', {
                key: 'test',
                value: ['Test'],
...
//# sourceMappingURL=translations.service.spec.js.map